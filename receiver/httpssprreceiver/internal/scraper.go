...
package internal // import "httpsspr/internal"
...
func (s *scraper) parseJSON(data []byte) map[string]any {
	...
	if err := json.Unmarshal(data, &tmp); err != nil {
		s.logger.Sugar().Errorln("Error unmarshalling JSON:", err)
		return metricsMap
	}
	...
	for _, obj := range tmp {
		...
		if !ok {
			s.logger.Sugar().Errorln("Invalid JSON object: missing or invalid 'name' field")
			continue
		}
		value, ok := obj["value"].(string)
		if !ok {
			s.logger.Sugar().Errorln("Invalid JSON object:", name, "- missing or invalid 'value' field")
			continue
		}
...
func (s *scraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	metricsAttributeMap := s.parseJSON(data)
	s.mb.RecordHttpjsonDurationDataPoint(pcommon.NewTimestampFromTime(time.Now()), time.Since(start).Milliseconds(), s.cfg.Endpoint, int64(res.StatusCode), "GET", metricsAttributeMap)
	s.mb.RecordHttpjsonDbUnavailableCountDataPoint(pcommon.NewTimestampFromTime(time.Now()), time.Since(start).Milliseconds(), s.cfg.Endpoint, int64(res.StatusCode), "GET", metricsAttributeMap)
