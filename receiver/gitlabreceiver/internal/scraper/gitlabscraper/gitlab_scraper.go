...
func (gls *gitlabScraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	if gls.cfg.Endpoint != "" {
		...
		graphCURL, err = url.JoinPath(gls.cfg.Endpoint, "api/graphql")
		if err != nil {
			gls.logger.Sugar().Errorf("error: %v", err)
		}

		restCURL, err = url.JoinPath(gls.cfg.Endpoint, "/")
		if err != nil {
			gls.logger.Sugar().Errorf("error: %v", err)
		}
	}
	...
	for _, project := range projectList {
		...
		go func() {
			...
			gls.mb.RecordVcsRefCountDataPoint(now, int64(len(branches.BranchNames)), url, path, projectID, refType)
			mux.Unlock()
			for _, branch := range branches.BranchNames {
				...
				if commit != nil {
					...
					mux.Lock()
					gls.mb.RecordVcsRefTimeDataPoint(now, int64(branchAge), url, path, projectID, branch, refType)
					mux.Unlock()
				}
			}
