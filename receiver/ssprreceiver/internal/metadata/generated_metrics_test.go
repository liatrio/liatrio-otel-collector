...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSsprDurationDataPoint(ts, 1, "http.url-val", 16, "http.method-val", map[string]any{"key1": "http.response.json-val1", "key2": "http.response.json-val2"})
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "sspr.duration":
					assert.False(t, validatedMetrics["sspr.duration"], "Found a duplicate in the metrics slice: sspr.duration")
					validatedMetrics["sspr.duration"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
