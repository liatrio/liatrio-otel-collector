...
func eventToTraces(event interface{}, config *Config, logger *zap.Logger) (ptrace.Traces, error) {
	...
	case *github.WorkflowJobEvent:
		logger.Info("Processing WorkflowJobEvent",
			zap.Int64("job_id", e.WorkflowJob.GetID()),
			zap.String("job_name", e.GetWorkflowJob().GetName()),
			zap.String("repo", e.GetRepo().GetFullName()))
	...
	case *github.WorkflowRunEvent:
		logger.Info("Processing WorkflowRunEvent",
			zap.Int64("workflow_id", e.GetWorkflowRun().GetID()),
			zap.String("workflow_name", e.GetWorkflowRun().GetName()),
			zap.String("repo", e.GetRepo().GetFullName()))
...
func createParentSpan(scopeSpans ptrace.ScopeSpans, steps []*github.TaskStep, job *github.WorkflowJob, traceID pcommon.TraceID, logger *zap.Logger) pcommon.SpanID {
	...
	if err != nil {
		logger.Error("Failed to generate job span ID", zap.Error(err))
	}
...
func createSpan(scopeSpans ptrace.ScopeSpans,
	step *github.TaskStep,
	job *github.WorkflowJob,
	traceID pcommon.TraceID,
	parentSpanID pcommon.SpanID,
	logger *zap.Logger,
	stepNumber ...int) pcommon.SpanID {
	...
	if len(stepNumber) > 0 && stepNumber[0] > 0 {
		spanID, err = generateStepSpanID(job.GetRunID(), int(job.GetRunAttempt()), job.GetName(), step.GetName(), stepNumber[0])
		if err != nil {
			logger.Error("Failed to generate step span ID", zap.Error(err))
			return pcommon.SpanID{}
		}
		span.Attributes().PutInt("ci.github.workflow.job.step.number", int64(stepNumber[0]))
	} else {
		spanID, err = generateStepSpanID(job.GetRunID(), int(job.GetRunAttempt()), job.GetName(), step.GetName())
		if err != nil {
			logger.Error("Failed to generate step span ID", zap.Error(err))
			return pcommon.SpanID{}
		}
