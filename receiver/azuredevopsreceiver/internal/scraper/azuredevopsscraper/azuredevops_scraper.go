...
func (ados *azuredevopsScraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	// Determine which API calls are needed based on enabled metrics
	m := ados.cfg.Metrics
	...
	if !needsRepos {
		ados.logger.Sugar().Debug("skipping repository API calls, no repository metrics enabled")
	} else {
		...
		if m.VcsRepositoryCount.Enabled {
			ados.mb.RecordVcsRepositoryCountDataPoint(now, int64(len(repos)))
		}

		if !needsRepoLoop {
			ados.logger.Sugar().Debug("skipping per-repo API calls, no per-repo metrics enabled")
		} else {
	...
	// Scrape deployment metrics if configured and any deployment metric is enabled
	if needsDeployments && ados.cfg.DeploymentPipelineName != "" && ados.cfg.DeploymentStageName != "" {
	...
	// Scrape work item metrics if enabled and any work item metric is enabled
	if needsWorkItems && ados.cfg.WorkItemsEnabled {
